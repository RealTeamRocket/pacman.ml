...
#endif
// Faster ticks for API mode training
#ifdef PACMAN_API_ENABLED
#undef TICK_DURATION_NS
#define TICK_DURATION_NS (1000000) // Much faster for training
#endif
...
typedef struct {
  actor_t actor;
  ghosttype_t type;
  dir_t next_dir;    // ghost AI looks ahead one tile when deciding movement
                     // direction
...
// the tiles for displaying the bonus-fruit-score, this is a number built from 4
// tiles
...
static const sound_desc_t snd_prelude = {.ptr = snd_dump_prelude,
                                         .size = sizeof(snd_dump_prelude),
                                         .voice = {true, true, false}};

static const sound_desc_t snd_dead = {.ptr = snd_dump_dead,
                                      .size = sizeof(snd_dump_dead),
                                      .voice = {false, false, true}};

static const sound_desc_t snd_eatdot1 = {.func = snd_func_eatdot1,
                                         .voice = {false, false, true}};

static const sound_desc_t snd_eatdot2 = {.func = snd_func_eatdot2,
                                         .voice = {false, false, true}};

static const sound_desc_t snd_eatghost = {.func = snd_func_eatghost,
                                          .voice = {false, false, true}};

static const sound_desc_t snd_eatfruit = {.func = snd_func_eatfruit,
                                          .voice = {false, false, true}};

static const sound_desc_t snd_weeooh = {.func = snd_func_weeooh,
                                        .voice = {false, true, false}};

static const sound_desc_t snd_frightened = {.func = snd_func_frightened,
                                            .voice = {false, true, false}};
...
sapp_desc sokol_main(int argc, char *argv[]) {
  ...
  if (api_mode) {
    fprintf(
        stderr,
        "Error: API mode requested but not compiled with PACMAN_API_ENABLED\n");
  }
#endif
  ...
  return (sapp_desc){
      ...
      .swap_interval = 0, // Disable Vsync for faster training
  };
}
...
static void frame(void) {
#ifdef PACMAN_API_ENABLED
  if (state.api.enabled) {
    // Process multiple steps per frame to speed up training
    // Process steps until we run out of frame time (target 60fps -> ~16ms)
  ...
  // Logic for standard game loop (non-API or timing preservation)
...
static void start(trigger_t *t) { t->tick = state.timing.tick + 1; }
...
static int2_t mul_i2(int2_t v, int16_t s) { return (int2_t){v.x * s, v.y * s}; }
...
static void vid_color_tile(int2_t tile_pos, uint8_t color_code,
                           uint8_t tile_code) {
  assert(valid_tile_pos(tile_pos));
  state.gfx.video_ram[tile_pos.y][tile_pos.x] = tile_code;
  state.gfx.color_ram[tile_pos.y][tile_pos.x] = color_code;
...
static void vid_color_text(int2_t tile_pos, uint8_t color_code,
                           const char *text) {
  assert(valid_tile_pos(tile_pos));
  uint8_t chr;
...
*/
static void vid_color_score(int2_t tile_pos, uint8_t color_code,
                            uint32_t score) {
...
*/
static void vid_draw_tile_quad(int2_t tile_pos, uint8_t color_code,
                               uint8_t tile_code) {
...
static void vid_fruit_score(fruit_t fruit_type) {
  ...
  uint8_t color_code =
      (fruit_type == FRUIT_NONE) ? COLOR_DOT : COLOR_FRUIT_SCORE;
  for (int i = 0; i < 4; i++) {
    vid_color_tile(i2(12 + i, 20), color_code,
                   fruit_score_tiles[fruit_type][i]);
  }
}
...
static sprite_t *spr_pacman(void) { return &state.gfx.sprite[SPRITE_PACMAN]; }
...
static sprite_t *spr_fruit(void) { return &state.gfx.sprite[SPRITE_FRUIT]; }
...
// set a debug marker
#if DBG_MARKERS
static void dbg_marker(int index, int2_t tile_pos, uint8_t tile_code,
                       uint8_t color_code) {
  assert((index >= 0) && (index < NUM_DEBUG_MARKERS));
  state.gfx.debug_marker[index] =
      (debugmarker_t){.enabled = true,
                      .tile = tile_code,
                      .color = color_code,
...
static void game_update_tiles(void) {
  ...
  static const int2_t pill_pos[NUM_PILLS] = {
      {1, 6}, {26, 6}, {1, 26}, {26, 26}};
  ...
  {
    int16_t x = 24;
    for (int i = ((int)state.game.round - NUM_STATUS_FRUITS + 1);
         i <= (int)state.game.round; i++) {
      if (i >= 0) {
  ...
  // scatter/chase/frightened: just head towards the current target point
    ...
    if ((dist_to_mid.x == 0) && (dist_to_mid.y == 0)) {
      ...
      for (int i = 0; i < NUM_DIRS; i++) {
        ...
        const int2_t test_pos =
            clamped_tile_pos(add_i2(lookahead_pos, dir_to_vec(dir)));
...
static void game_update_actors(void) {
  ...
  if (game_pacman_should_move()) {
    ...
    if (state.game.active_fruit != FRUIT_NONE) {
      const int2_t test_pos =
          pixel_to_tile_pos(add_i2(actor->pos, i2(TILE_WIDTH / 2, 0)));
...
api_game_state_t api_game_get_state_internal(void) {
  ...
  // Fruit info
  result.fruit.active = now(state.game.fruit_active) &&
                        !after(state.game.fruit_active, FRUITACTIVE_TICKS);
...
void api_game_start(void) { api_game_start_internal(); }

void api_game_restart(void) { api_game_restart_internal(); }
...
static void gfx_create_resources(void) {
  ...
  // target
  state.gfx.offscreen
      .pip = sg_make_pipeline(&(sg_pipeline_desc){.shader =
                                                      sg_make_shader(
                                                          &(sg_shader_desc){
                                                              .attrs =
                                                                  {
  ...
  // create the image, texture- and color-attachment view for the rendered
  // result
  ...
  state.gfx.tilerom.img = sg_make_image(
...
static void gfx_draw(void) {
  ...
  sg_update_buffer(
      ...
      &(sg_range){.ptr = state.gfx.vertices,
                  .size = state.gfx.num_vertices * sizeof(vertex_t)});
...
// the snd_voice_tick() function updates the Namco sound generator and must be
// called with 96 kHz
