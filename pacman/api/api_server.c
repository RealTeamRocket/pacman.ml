...
static api_direction_t parse_direction(const char *dir_str) {
    if (strcmp(dir_str, "up") == 0) return API_DIR_UP;
    if (strcmp(dir_str, "down") == 0) return API_DIR_DOWN;
    if (strcmp(dir_str, "left") == 0) return API_DIR_LEFT;
    if (strcmp(dir_str, "right") == 0) return API_DIR_RIGHT;
    if (strcmp(dir_str, "none") == 0 || strcmp(dir_str, "noinput") == 0) return API_DIR_NONE;
    return API_DIR_NONE;
}

// Build state JSON response
static void build_state_json(char *buffer, size_t buffer_size, const api_game_state_t *state) {
    char *ptr = buffer;
    ...
    // Pacman
    written = snprintf(ptr, remaining,
        "\"pacman\":{\"x\":%d,\"y\":%d,\"dir\":%d,\"alive\":%s,\"just_died\":%s},",
    ...
    for (int i = 0; i < 4; i++) {
        written = snprintf(ptr, remaining,
            ...
            state->ghosts[i].state, state->ghosts[i].type,
            (i < 3) ? "," : "");
        ptr += written; remaining -= written;
    }
    ...
    written = snprintf(ptr, remaining,
        ...
        state->fruit.active ? "true" : "false",
        state->fruit.x, state->fruit.y, state->fruit.type);
    ptr += written; remaining -= written;
        ...
        // POST /api/step - Step the game with direction
            ...
            if (mg_strcmp(hm->method, mg_str("POST")) == 0) {
                ...
                if (mg_json_get_num(json, "$.direction", &dir_num)) {
                    ...
                    if (dir < 0 || dir > API_DIR_RIGHT) dir = API_DIR_NONE;
                    api_game_step(dir);
                } else {
...
bool api_server_init(const char* port) {
    ...
    printf("  POST /api/step     - Step the game (body: {\"direction\": \"up|down|left|right|none\"})\n");
